...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Tedcoin: starting difficulty is 1 / 2^12
...
{
    // Tedcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / NDIFF_START_DIGISHIELD); // Tedcoin: 280k blocks in ~4 months

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 108 * 40; // Tedcoin: 3.5 days
static const int64 nTargetTimespanNEW = 40 ; // Tedcoin: 40 seconds
static const int64 nTargetSpacing = 1 * 40; // Tedcoin: 40 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Tedcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Tedcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Tedcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Tedcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TedcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TedcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TedcoinMiner(CWallet *pwallet)
{
    printf("TedcoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running TedcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TedcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TedcoinMiner, pwallet));
}
